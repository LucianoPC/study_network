...
begin
  user_ip = `sudo ifconfig`.split("\n\n").select{|line| line.start_with?(network_type)}.first.split("\n").select{|line| line.strip.start_with?("inet addr:")}.first.strip.split[1].split(':')[1]
rescue
